...
正式配置是一个包含八个时刻的 standalone 自动化。每次触发创建一个短生命周期节点任务；不存在跨日复用的“持久调度器”。节点按实际北京时间计算最近到期时刻，再调用 `dispatch_node_claim`，所以 Codex 恢复后即使多个错过的触发同时补跑，也只有一个任务能认领最近节点，其余立即跳过。
