...
2. **Dispatcher**：通过 `dispatch_node_claim` 原子判断当前唯一可执行节点；同步校验每日任务的实际归档状态，等待带 ID 的短 ACK，再发起独立分析回合。
...
- 本地任务登记不是 Codex 任务运行状态的事实来源；每次投递必须实际解归档并验证目标可访问。解归档后的 `notLoaded` 是可接收新消息并冷启动的正常状态，不等同于任务缺失。
